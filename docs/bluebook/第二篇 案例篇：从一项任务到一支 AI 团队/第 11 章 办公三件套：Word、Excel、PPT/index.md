...
> 质量门禁补录：以一组虚构的“办公区域月度能耗数据”为例，交付物包含情况说明、数据核对表与五页汇报。WorkBuddy 负责检查缺失值、生成趋势图并组织汇报，但异常原因须由经办人核实。
...
输入文件：{文件名}
输出对象：{读者}
交付要求：{Word/Excel/PPT 及格式}
统计口径：{口径；没有则写“待确认”}
参考模板：{文件名；没有则写“未提供”}

请先列出材料缺口、风险与交付清单，给出提纲/处理规则/故事线，待我确认后再生成文件，并保留原始数据、不覆盖原件。
```

## 常见弯路

- 直接生成三份终稿：跨文件数字容易打架，应先确认共同事实表。
- 在原表上直接清洗：错误难回退，原始表只读、处理表另存。
- PPT 只追求版式：结论失去证据，每张图注明口径与来源。
